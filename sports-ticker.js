...
//
// Requires an element with id="sports-ticker-inner" on the page.
// Call loadSports() to run once, and set up your own refresh interval
// (e.g. setInterval(loadSports, 300000)) from each dashboard's own script.
// ══════════════════════════════════════════════════════════════════════════
...
function parseESPNGames(data,teamAbbr){
  ...
  return events.map(e=>{
    ...
    // Only map explicitly recognized statuses — anything unrecognized was
    // previously defaulting to 'live', which incorrectly hijacked the whole
    // ticker display (the live-game branch returns early, skipping past the
    // recent/next-game sections entirely) whenever ESPN reported a status
    // string other than exactly STATUS_FINAL or STATUS_SCHEDULED.
...
async function loadSports(){
  ...
  try{
    ...
    // Self-healing cache: if any ONE of the three parallel fetches above has a
    // rough moment, that specific piece (e.g. Cubs' "recent" game, or Bears'
    // "next" game) has no other source to fall back on within this cycle alone.
    // Merging in what we successfully learned last cycle heals over that —
    // fresh data always wins when available, but a transient gap no longer
    // silently erases a piece we already knew.
    ...
    function mergeWithCache(freshGames,cacheKey){
      // Fresh data ALWAYS wins for any game it actually reports on — priority-based
      // comparison here was a real bug: a game cached while 'live' would outrank
      // the same game once fresh data correctly reported it 'closed', permanently
      // stranding the ticker on a stale in-progress snapshot. Cache now only fills
      // in games that are completely MISSING from fresh data this cycle (a genuine
      // fetch gap), never overrides a game fresh data actually returned.
    ...
    function buildTeamTicker(name,logo,games,teamAbbr,standings){
      ...
      parts.push('<span style="color:#FFFFFF;font-weight:800;display:inline-flex;align-items:center;gap:4px;vertical-align:middle;">'+logo+' '+name.toUpperCase()+'</span>');
      // Weekly sports (NFL) need a much longer "recent game" lookback than daily
      // sports (MLB) — otherwise a Sunday result disappears mid-week even though
      // it's still the most recent relevant game.
    ...
    // last successful build had (e.g. a team's data transiently came back empty),
    // keep showing the last good ticker instead of silently dropping that team —
    // same protective pattern already used for Headlines and Calendar.
    ...
    if(window._lastGoodTickerCount!==undefined && populatedCount<window._lastGoodTickerCount){
      return; // keep whatever's already on screen; don't regress to fewer teams shown
    }
